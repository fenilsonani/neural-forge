...
def download_file(url: str, destination: Path, chunk_size: int = 8192, timeout: int = 300) -> None:
...
def load_pretrained_weights(
    ...
    for key, value in converted_state.items():
        if key in model_params:
            ...
            if param.shape != value.shape:
                raise ModelError(
                    f"Shape mismatch for {key}: " f"model {param.shape} vs weights {value.shape}"
                )
            param.data = value
        else:
    ...
    if strict and (missing_keys or unexpected_keys):
        raise ModelError(
            f"Strict loading failed. " f"Missing: {missing_keys}, Unexpected: {unexpected_keys}"
        )
...
def create_model_card_from_model(model: Module, name: str, description: str, **kwargs) -> ModelCard:
    ...
    return ModelCard(name=name, description=description, architecture=architecture, **kwargs)
