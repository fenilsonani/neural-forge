    ...
    try:
        ...
        if "WORLD_SIZE" in os.environ and "RANK" in os.environ:
            ...
            if world_size > 1:
                ...
                logger.info(
                    f"Distributed training initialized: rank={rank}, world_size={world_size}, backend={backend}"
                )
                return True
...
def make_distributed(
    model: Module, auto_setup: bool = True
...
def auto_scale_batch_size(base_batch_size: int) -> int:
    ...
    if is_initialized():
        ...
        logger.info(
            f"Scaled batch size from {base_batch_size} to {scaled_batch_size} for {world_size} processes"
        )
...
def auto_scale_learning_rate(base_lr: float, scaling_rule: str = "linear") -> float:
    ...
    if is_initialized():
        ...
        logger.info(
            f"Scaled learning rate from {base_lr} to {scaled_lr} using {scaling_rule} scaling"
        )
        return scaled_lr
